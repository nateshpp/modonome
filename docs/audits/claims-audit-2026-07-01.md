...
|---|---|---|
| "CI runs 'the config and packet validators'... Only drift/style/ratchet/AgentProof run in `ci.yml`" | `.github/workflows/ci.yml`'s `ratchet` job runs 17 gate-like steps as of this PR's final commit: snapshot freshness, governance-relevant-change assertion, drift guard, style check, repo hygiene, markdown governance, architecture drift, self-application conformance, license/adapter-boundary, prompt behavioral regression, learning traceability, promotion readiness, work-item validation, checker engagement, tests, AgentProof, and the ratchet itself. (The architecture-drift and self-application gates are new in this same PR; an earlier draft of this row undercounted them, an irony not lost on a document about catching exactly that.) `scripts/check-work-items.mjs` (CI-wired) imports and calls `validateWorkItem` from `validate-work-item.mjs` directly, so that named validator runs in CI, beyond being merely defined. | CLOSED |
