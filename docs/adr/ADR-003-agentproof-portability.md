...
2. `agentproof/runner.mjs:98-99` prints "This implementation meets AgentProof Level 3
   ...
   but is absent from `agentproof/README.md`, which uses UNHARDENED/PARTIAL/HARDENED.
...
3. The PARTIAL threshold in `runner.mjs:99` is set at >= 8 scenarios passing, but
   ...
   8/16 would be labeled PARTIAL by the runner but UNHARDENED by the spec.
...
3. Remove "Level 3" language from runner output; replace with the
   UNHARDENED/PARTIAL/HARDENED taxonomy already defined in `agentproof/README.md`. (WI-004)
...
The score modonome reports (16/16 HARDENED) remains valid as the reference
