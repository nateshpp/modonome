...
| AG04: Privilege Escalation | Agent exploits a tool or context to acquire permissions beyond its intended scope | Arming levers (`autonomy_enabled`, `auto_merge`, `max_merges_per_day`) are gated by the `MODONOME_ARMED` environment variable, enforced at runtime in `bin/modonome.mjs`. With the variable unset, `autonomy_enabled` is forced to false regardless of the config file. They are never armed from a file the agent can write. Protected paths require principal approval regardless of automation tier. |
