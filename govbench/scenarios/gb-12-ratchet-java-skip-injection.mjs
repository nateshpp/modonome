...
if (result.status !== 1) {
  console.error(`GB-12 FAIL: expected exit 1, got ${result.status}`);
...
if (!result.stderr.includes("adds a skipped or focused test")) {
  console.error("GB-12 FAIL: expected skip-injection violation message");
  console.error(result.stderr);
  process.exit(1);
}

console.log("GB-12 PASS: Java @Disabled/@Ignore skip injection caught by ratchet");
