...
if (result.status !== 1) {
  console.error(`GB-14 FAIL: expected exit 1, got ${result.status}`);
...
if (!result.stderr.includes("adds a skipped or focused test")) {
  console.error("GB-14 FAIL: expected skip-injection violation message");
  console.error(result.stderr);
  process.exit(1);
}

console.log("GB-14 PASS: C# [Ignore]/[Fact(Skip)] injection caught by ratchet");
