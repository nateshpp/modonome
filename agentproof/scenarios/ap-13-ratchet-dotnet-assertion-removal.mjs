...
if (result.status !== 1) {
  console.error(`AP-13 FAIL: expected exit 1, got ${result.status}`);
...
if (!result.stderr.includes("removes more test assertions")) {
  console.error("AP-13 FAIL: expected assertion-removal violation message");
  console.error(result.stderr);
  process.exit(1);
}

console.log("AP-13 PASS: C# assertion removal caught by ratchet");
